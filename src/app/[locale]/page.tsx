...
import { getAthletes, getQuizzesByAthleteId } from "@/lib/data";
        ...
        <div className="mx-auto max-w-3xl px-6 py-16">
          <div className="space-y-12">
            {athletes.map((athlete, index) => {
              const hasQuiz = getQuizzesByAthleteId(athlete.id, loc).length > 0;
              return (
              <div key={athlete.id}>
                <Link
                ...
                {hasQuiz && (
                  <div className="mt-2 flex justify-end px-2">
                    <Link
                      href={`/${loc}/athletes/${athlete.id}/quiz`}
                      className="rounded-full border border-accent/30 bg-[var(--glass-bg)] backdrop-blur-sm px-4 py-1.5 text-xs font-medium text-accent hover:bg-accent/10 hover:border-accent/50 transition-colors"
                    >
                      {dict.home.takeQuiz}
                    </Link>
                  </div>
